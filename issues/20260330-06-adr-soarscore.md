# ADR: SoarScore Integration Approach

## Feature

Document the architectural decision for how Compman Mobile fetches task files from soarscore.com — specifically why HTML scraping is used, what was considered, and what the review trigger is.

## Scope

Create `docs/adr/005-soarscore-html-scraping.md` only. No code changes.

## Dependencies

None. Can be done at any time, but ideally before or alongside `20260330-01-soarscore-data-layer.md`.

---

## Tasks

Create `docs/adr/005-soarscore-html-scraping.md` with:
...
- soarscore.com is the established source used by the TUI version (openvario-compman).
- SoarScore has no public API. The only machine-readable output is the `#Downloads` tab on competition pages, which contains `<a download href="...">` links with structured text descriptions.
...
| soaringspot.com task download | SoaringSpot serves tasks in a different format (not `.tsk` directly); SoarScore already generates XCSoar-compatible `.tsk` XML |
...
**Review trigger:** If SoarScore publishes a public API, migrate to it and update this ADR.

---

## Acceptance Criteria

1. `docs/adr/005-soarscore-html-scraping.md` exists and is coherent.
