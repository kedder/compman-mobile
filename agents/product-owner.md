...
`add_competition_updated/`). Each directory holds a `screen.png` (visual reference) and a `code.html`
(annotated HTML prototype). Check for mockups relevant to the story's affected screens.
...
- Check `docs/design/` for mockup directories that cover those screens. If a relevant
  mockup exists, read its `code.html` and note it for the Planner; it is the authoritative
  visual spec for that screen.
