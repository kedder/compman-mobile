...
- ✅ **Fly XCSoar button** — full-width "Fly XCSoar" `ElevatedButton.icon` added to the Task card, shown only when the installed task version matches the fetched one (i.e. the "NEW UPDATE" badge is hidden); new `launchPackage` Android bridge method + `XcsoarSafService.launchPackage`; new `activeFlavorPackageIdProvider` resolving the active flavor via `resolveFlavorPackageId`; new `AppButtonStyles.success` (64dp, green); disabled states cover "Set up XCSoar folder first." and "XCSoar is not installed."; launch failures append a dismissible error banner; widget tests cover all visibility/enablement states and the error path; `docs/features/xcsoar.md` and `docs/features/competitions.md` updated
...
- ✅ **Flight log email domain/data layer** (issue `20260727-01-flight-log-domain-and-saf-read.md`) — `scoringEmail` field added to `BookmarkedCompetition` (freezed) and `BookmarkedCompetitionModel` (HiveField 11); `setCompetitionScoringEmail` on repository interface and impl, `SetCompetitionScoringEmail` use case, DI wired; new `FlightLogFile` entity (transient, not persisted); `listFlightLogs` Android bridge method (lists `.igc` files under `logs/` in the granted SAF tree, returns empty list if no `logs/` folder) + `XcsoarSafService.listFlightLogs()` (returns raw maps to keep `core/platform/` free of feature-domain imports); `GetTodaysFlightLogs` use case maps and filters to today's date; `todaysFlightLogsProvider` shared presentation provider; unit tests for use case, repository method, and Hive round-trip. Screens/panel land in issues 2 and 3 of the same batch.
- ✅ **Flight Log screen** (issue `20260727-02-flight-log-screen.md`) — new `FlightLogScreen` (`ConsumerStatefulWidget`) reachable at `/competitions/:id/flight-logs`; lists today's `.igc` files as pre-checked `CheckboxListTile` rows, a recipient email field pre-filled from `BookmarkedCompetition.scoringEmail`, and a full-width "Send" button disabled until at least one file is checked and the email looks valid; `SendFlightLogs` use case shares files via the SAF bridge then persists the recipient as the scoring email; `shareFlightLogs` Android bridge method (`ACTION_SEND_MULTIPLE` with `message/rfc822` type, `EXTRA_STREAM`/`clipData` for multiple attachments, `FLAG_GRANT_READ_URI_PERMISSION`) + `XcsoarSafService.shareFlightLogs()`; `sendFlightLogsProvider` DI wired; inline error shown only for `NO_MAIL_APP`; unit tests for the use case and widget tests for the screen; `docs/features/competitions.md` and `docs/features/xcsoar.md` updated. Not yet reachable by users — the Competition Detail entry point lands in issue 3 of the same batch.
- ✅ **Flight Logs panel** (issue `20260727-03-flight-logs-panel.md`) — completes the 3-issue flight-log-email batch. New `FlightLogsPanel` widget, embedded at the bottom of the Task card's footer (below the Fly XCSoar button, no border of its own — it's plain content inside the card's tinted footer); watches `todaysFlightLogsProvider`, showing a muted "No flight logs recorded today" message (also used as the fallback for any provider error, including `SAF_NOT_CONFIGURED` — a passive check should never interrupt the pilot) or an `IconMetaRow` "N flight log(s) recorded" line (same subdued style as the "Generated ..." row above it) with a full-width "Email flight logs" `OutlinedButton.icon` below it (envelope icon, white/outlined via the new `AppButtonStyles.outlinedFullWidth`, sized to match "Download task") pushing `/competitions/:id/flight-logs`. Living inside the Task card means the summary now only appears once a class is selected and that class's task has loaded, same as the Task/Airspace/Waypoints cards. `_CompetitionDetailScreenState` mixes in `WidgetsBindingObserver` and invalidates `todaysFlightLogsProvider` on `AppLifecycleState.resumed`; also added to the pull-to-refresh and AppBar-refresh invalidation lists. On the Flight Log screen, each file row is a custom `_FlightLogCheckboxRow` (a plain `Checkbox` + filename `Text` in a `Row`, tap-toggle via `InkWell`) instead of `CheckboxListTile`, so the checkbox lines up with the Send button/email field's left edge instead of sitting behind `ListTile`'s extra content padding. Widget tests cover singular/plural/zero/error text, the Email navigation, the pre-class-selection hidden state, and both refresh paths; `docs/features/competitions.md` and `CHANGELOG.md` updated.
